...
**Verified working (2026-07-28):** both SMS and WhatsApp were confirmed with real test sends — mNotify and waapi.app both returned real success responses (message IDs, delivery confirmation).

If WhatsApp ever stops sending again, check for `"Instance is not ready (status: qr)."` in the `sendWhatsApp()` response — that means the WhatsApp Business session on waapi.app's dashboard has disconnected (this can happen if the linked phone goes offline too long, similar to WhatsApp Web) and needs to be reconnected by scanning a QR code there; it's not a code issue. You can check current status without the dashboard via `GET https://waapi.app/api/v1/instances/{instance_id}/client/status` (Bearer auth with `WHATSAPP_APIKEY`) — `instanceStatus` will read `"qr"` when disconnected, `"ready"` when working. SMS is independent of this and unaffected either way.
